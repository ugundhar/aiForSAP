...
try:
    ...
    from tools.Retriever_technical_objects import retrieve_and_query_llm
    from tools.createNotification import post_notification
    from tools.orderDetails import get_order_by_number
    # from tools.orderapi import create_order
    from tools.orderCreation import create_order
    from langgraph.graph import StateGraph, START, END
    from langgraph.prebuilt import ToolNode
    from langgraph.graph.message import add_messages
    from typing import Annotated
...
def initialize_system():
    ...
    try:
        with st.spinner("🔄 Initializing SAP AI Core connection..."):
            ...
            def createOrder(MaintenanceOrderType:str,
                            MaintenanceOrderDesc:str,
                            MainWorkCenter:str,
                            MaintenancePlanningPlant:str
                            )->str:
                """Tool function to create the orders from SAP system."""
                payload={
            ...
            ##order details
            def getOrderDetails(MaintenanceOrder: str) -> str:
                 """Tool function to get the order detials from SAP system."""
                 result=get_order_by_number(MaintenanceOrder)
                 return json.dumps(result)
            ...
            tools = [multiply, openNotifications, retrieveTechnicalObjects, createNotification,getOrderDetails,createOrder]
            llm_with_tools = llm.bind_tools(tools)
